...
$aprovacaoHandle = Sistema::getPost('check');
...
try {
    ...
    for ($i = 0; $i < count($_POST['check']); $i++) {
        $check = $_POST['check'];
        $params = array("aprovacao" => $check[$i]);

        $result = $clientSoap->__soapCall("AprovarAprovacao", array("AprovarAprovacao" => array("aprovacao" => $params)));

        $retorno = $result->AprovarAprovacaoResult;
    ...
    $check = NULL;
    $_SESSION['voltou'] = 'voltou';

    echo "<script language='javascript' type='text/javascript'>history.back();self.location.reload();</script>";
