...
try {
	...
	for ($i = 0; $i < $arquivoCount; $i++) {
        ...
        if (isset($_FILES["image_src"]["name"][$i])) {
			...
			if(filesize($arquivo) > (4 * 1000 * 1000)){
				...
				$anexos["listaAnexoViagemDespesa"]["InserirAnexoViagemDespesa"][] = $params;
    ...
    $result = $clientSoap->__soapCall("InserirAnexoViagemDespesa", array("InserirAnexoViagemDespesa" => $anexos));
	
    $retorno = $result->InserirAnexoViagemDespesaResult;
