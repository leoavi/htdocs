...
function multiselection() {
    $('#tipo,#cliente,#processo,#transportadora').multiselect({
        columns: 1,
        search: true
    });
};
...
function adicionarOnClickConsulta(){
    $('#tableMinhasCargasDescargas td').dblclick(function () {
        $('#tableMinhasCargasDescargas td').removeClass("activetr");
        $(this).addClass("activetr");
        $(this).parent('tr').find('[name="check[]"]').prop('checked', true);
    
        $('input:radio').each(function () {
            if ($(this).is(':checked')) {
                window.location.href = '../../view/armazenagem/AnexoCargaDescarga.php?cargaDescarga=' + parseInt($(this).val());
            }
        });   
...
function limparFiltro() {
    ...
    $('#tipo,#cliente,#processo,#transportadora').val('');
    $('#nrcontrole,#nrpedido,#nrordem').val('');
...
function aplicarFiltro() {  
    ...
    $('#loader').show();
    $('#tableMinhasCargasDescargas tbody').empty();
    $('#registroNaoEncontrato').hide();
    
    $.post('/controller/armazenagem/MinhasCargasDescargasRequest.php', { 
        'REQUEST' : 'getRegistro', 
        'FILTRO' : { 
            'TIPO' : $('#tipo').val(),
        ...
        if(res) {                
            if(res.DADOS.length > 0) {                
                $('#tableMinhasCargasDescargas tbody').html(res.DADOS.join(''));           
            }
            else {
