...
$(document).ready(function () {
    $('#reqtablenew tr').click(function () {
		...
		$('button.botaoItens').removeAttr('disabled');
...
$('#aplicacao').focus(function() {
    $('#aplicacao').autocomplete('option', 'minLength', 0);
    $('#aplicacao').autocomplete('search', $('#aplicacao').val());
});
...
function buscavalorunitario(tabela,lista, itemhandle){
   ...
   var url = "../../controller/comercial/InserirItemPedidoDeVendaRecuperaAlmoxarifado.php";
	...
	var url = "../../controller/comercial/InserirItemPedidoDeVendaRecuperaAplicacao.php";
			
    $.getJSON(url , function(data){
         var dados = [];
		 var handle = [];

		 $('#aplicacao').autocomplete({ 
		 
			 source: data, 
			 minLength: 0,
			 select: function(event, ui) {
				 $("#aplicacaoHandle").val(ui.item.id);
			 },
			 autoFocus: true  
		 });
	});
...
$(document).ready(function(){
$("#FormItemPedido :text").add('#data').add('#obs').focus(function() {
    var valor = $(this).val();
	
$("#FormItemPedido :text").add('#data').add('#obs').blur(function() {
...
if(editou === 'S'){
  	...
  	if($(this).val() === valor){
		...
		$('#excluirPedidoMobile').removeClass('display');
		$('#liberarPedidoMobile').removeClass('display');
		$('#voltarPedidoMobile').removeClass('display');
		$('#cancelarPedidoMobile').removeClass('display');
	}
	else{
		document.getElementById('toggle').innerHTML = "";
